...
#dat.diffvar= data.frame(estimate=c(pes.ft.mal1$b, pes.ft.mal2$b),stderror=c(pes.ft.mal1$se, pes.ft.mal2$se),moderatora = c("mal1","mal2"), tau2=round(c(pes.ft.mal1$tau2, pes.ft.mal2$tau2),3))
...
metareg.mod1=rma(yi,vi,data=ies.ft,mods=~moderator1,method="REML",test="z")
print(metareg.mod1)
```
...
plot(ies.ft$moderator1,transf.ipft.hm(ies.ft$yi,targ=list(ni=dat$total)),cex=size,pch=20,xlab="Moderator 1", ylab="Proportion",las=1)
lines(0:40,pes.bb$pred)
lines(0:40,pes.bb$ci.lb,lty="dashed")
lines(0:40,pes.bb$ci.ub,lty="dashed")
ids <- c(1:6)
pos <- c(3)
text(ies.ft$moderator1[ids], transf.ipft.hm(ies.ft$yi,targ=list(ni=dat$total))[ids], ids, cex=0.9, pos=pos)
```
