...
dev.off()
```
```{r Calculating subgroup summary proportions, conducting subgroup analysis, and recalculating summary proportion}
#Assuming a common between-study variance component across subgroups
#Pool within-group estimates of tau-squared (between-study variance):0.4427 is the combined tau^2 resulting from by pooling 0.93224 and 0.24743
...
pes.logit.birthcohort=subganal.studydesign=rma(yi,vi,data=ies.logit,mods=~studydesign=="Others",method="DL")
pes.logit.others=subganal.studydesign=rma(yi,vi,data=ies.logit,mods=~studydesign=="Birth cohort",method="DL")
pes.subg=predict(subganal.studydesign,transf=transf.ilogit)
...
#print(pes.subg[order(pes.subg$pred)],digits=6)
print(subganal.studydesign,digits=4)
print(pes.subg[1],digits=6);print(pes.subg[17],digits=6)
print(pes.studydesign,digits=6)
```
```{r Calculating subgroup summary proportions}
#Separate Meta-Analyses: not assuming a common between-study variance component across subgroups
...
print(pes.birthcohort,digits=6);print(pes.logit.birthcohort,digits=5);confint(pes.logit.birthcohort,digits=5)
print(pes.others,digits=6);print(pes.logit.others,digits=5);confint(pes.logit.others,digits=5)
```
```{r Conducting subgroup analysis and recalculating summary proportion}
#Separate Meta-Analyses: not assuming a common between-study variance component across subgroups
...
print(pes.studydesign,digits=6)
```
```{r Conducting subgroup analysis not assuming a common between-study variance component across subgroups}
#This chunk can get exactly the same results as above
dat.diffvar=data.frame(estimate=c(coef(pes.logit.birthcohort),coef(pes.logit.others)), 
                       stderror=c(pes.logit.birthcohort$se, pes.logit.others$se),
                       meta=c("Birth cohort","Others"),
                       tau2=round(c(pes.logit.birthcohort$tau2, pes.logit.others$tau2),3))
subganal.studydesign=rma(estimate,sei=stderror,mods=~meta,method="FE",data=dat.diffvar)
pes.logit.studydesign=rma(estimate,sei=stderror,method="FE",data=dat.diffvar)
pes.studydesign=predict(pes.logit.studydesign,transf=transf.ilogit)
print(dat.diffvar)
print(subganal.studydesign,digits=4)
