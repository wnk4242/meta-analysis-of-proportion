...
```
```{r Finding outliers with residuals}
...
subganal.studytype=rma(estimate,sei=stderror,mods=~studytype,method="FE",data=dat.diffvar,digits=2)
print(subganal.studytype,digits=2)
```
```{r Scatter plot for study type}
...
```
```{r Scatter plot for sample size}
...
predict(pes.trimfill,transf=transf.ilogit,digits=5)
pes.trimfill
#If you want the x-axis to be expressed as a logit transformed proportion, then remove the "#" sign before the following line.
funnel(pes.trimfill)
#If you want the x-axis to be expressed as a proportion, then remove the "#" sign before the following line.
