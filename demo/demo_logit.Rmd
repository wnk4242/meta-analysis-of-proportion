...
install.packages(c("metafor","meta","weightr"))#When you use knitr,delete this chunk
```
```{r Loading packages}
library(metafor)
library(meta)
...
```
```{r Reading in data}
dat=read.csv("data.csv",header=T,sep=",")#When this rmd file is created in a R project, the default directory is the path to the current folder/note that when data is imported into r, if your column names contain letters in uppercase, they will be converted to lowercase, which means you are not able to use uppercase/lowercase letters to distinguish two columns. You need to use different words to distinguish them. In additions, you cannot place a blank in between two words. For instance, the correct form for column names should be: worldregion instead of world region. 
```
...
pes.summary=metaprop(cases,total,authoryear,data=dat,byvar=studytype,sm="PLO",method.ci="NAsm",method.tau="DL",incr=0.5,allincr=FALSE,addincr=FALSE,title="")
...
regtest(pes.logit,model="lm",predictor="sei")#warning: when using knitr, "'" in Egger's should be deleted
```
```{r Rank correlation}
ranktest(pes.logit)
```
