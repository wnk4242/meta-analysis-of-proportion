...
dev.off()
```
```{r Calculating subgroup summary proportions according to study design}
#not assuming a common between-study variance component across subgroups
...
```
```{r Subgroup analysis not assuming a common between-study variance component across subgroups}
#Separate Meta-Analyses
#Do not pool within-group estimates of tau-squared
#This is the option used by RevMan
...
```{r Subgroup analysis not assuming a common between-study variance component across subgroups}
#Separate Meta-Analyses
#This chunk can get exactly the same results as above but only REML is available
rma.mv(yi,vi,mods=~studydesign,random=~studydesign|author,struct="DIAG",data=ies.logit,digits=5)
```
```{r Subgroup analysis assuming a common between-study variance component across subgroups}
...
#subganal.samplesize=rma(yi,vi,data=ies.logit,mods=~samplesize,method="DL")
#print(subganal.samplesize,digits=4)
#predict(subganal.samplesize,transf=transf.ilogit,digits=6)
subganal.studydesign=rma(yi,vi,data=ies.logit,mods=~studydesign,method="DL")
print(subganal.studydesign,digits=4)
predict(subganal.studydesign,transf=transf.ilogit,digits=6)
```
