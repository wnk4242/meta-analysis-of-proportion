...
subganal.guangs=rma(estimate, sei=stderror, mods =~province, method="FE", data=dat.diffvar)
print(subganal.guangs)
```
```{r Scatter plot for province}
wi=1/sqrt(ies$vi)
size=1+3*(wi-min(wi))/(max(wi)-min(wi))
plot(ies$prov,ies$yi,cex=ies$prov,pch=1,las=1,xlab="Province", ylab="Proportion")
```
```{r Funnel plot}
funnel(pes)
```
