...
wi=1/sqrt(ies.ft$vi)
size=1+3*(wi-min(wi))/(max(wi)-min(wi))
pes.ft=predict(metareg.mod1,newmods=c(0:40),transf=transf.ipft.hm,targ=list(ni=dat$total))
plot(ies.ft$moderator1,transf.ipft.hm(ies.ft$yi,targ=list(ni=dat$total)),cex=size,pch=1,xlab="Moderator 1", ylab="Proportion",las=1)
lines(0:40,pes.ft$pred)
lines(0:40,pes.ft$ci.lb,lty="dashed")
lines(0:40,pes.ft$ci.ub,lty="dashed")
ids <- c(1:6)
pos <- c(3)
