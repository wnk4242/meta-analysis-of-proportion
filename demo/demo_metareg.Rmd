...
ids=c(1:6)
pos=c(1)
text(ies$month[ids], transf.ilogit(ies$yi)[ids], ids, cex=0.9, pos=pos)
```
...
metareg.month=rma(yi,vi,data=ies.logit,mods=~month,method="REML",test="z")
print(metareg.month)
```
...
size=1+3*(wi-min(wi))/(max(wi)-min(wi))
preds.month=predict(metareg.month,newmods=c(0:40),transf=transf.ilogit)
plot(ies.logit$month,transf.ilogit(ies.logit$yi),cex=size,pch=1,xlab="Month", ylab="Proportion",las=1)
lines(0:40,preds.month$pred)
lines(0:40,preds.month$ci.lb,lty="dashed")
lines(0:40,preds.month$ci.ub,lty="dashed")
ids=c(1:6)
pos=c(1)
text(ies.logit$month[ids], transf.ilogit(ies.logit$yi)[ids], ids, cex=0.9, pos=pos)
```
```{r Scatterplot with logit transformed data}
...
size=1+3*(wi-min(wi))/(max(wi)-min(wi))
preds.month=predict(metareg.month,newmods=c(0:40))
plot(ies.logit$month,ies.logit$yi,cex=size,pch=1,xlab="Month", ylab="Logit event rate",las=1)
lines(0:40,preds.month$pred)
lines(0:40,preds.month$ci.lb,lty="dashed")
lines(0:40,preds.month$ci.ub,lty="dashed")
ids=c(1:6)
