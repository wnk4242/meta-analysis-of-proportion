...
pes.forest=metaprop(cases,total,author,data=dat,byvar=procedure,sm="PFT",method.ci="NAsm",method.tau="DL",incr=0.5,allincr=FALSE,addincr=FALSE,title="")
png("forestplot.png",width=1000,height=1000)
forest(pes.forest,
...
print(subganal.procedure)
```
```{r Scatter plot for procedure}
#If you want the y-axis to be expressed as a double arcsine transformed proportion
wi=1/sqrt(ies.da$vi)
procedure=1+3*(wi-min(wi))/(max(wi)-min(wi))
plot(ies.da$proced,ies.da$yi,cex=ies.da$proced,pch=1,las=1,xlab="Procedure", ylab="Double arcsine transformed proportions")
```
```{r Scatter plot for procedure}
#If you want the y-axis to be expressed as a proportion
wi=1/sqrt(ies.da$vi)
procedure=1+3*(wi-min(wi))/(max(wi)-min(wi))
plot(ies.da$proced,transf.ipft.hm(ies.da$yi,targ=list(ni=dat$total)),cex=ies.da$proced,pch=1,las=1,xlab="Procedure", ylab="Proportions")
```
```{r Funnel plot}
