...
pes.summary=metaprop(cases,total,author,data=dat,byvar=moderatora,sm="PFT",method.ci="NAsm",method.tau="DL",incr=0.5,allincr=FALSE,addincr=FALSE,title="")
png("forestplot.png",width=1000,height=1000)
forest(pes.summary,
...
subganal.moda=rma(yi,vi,data=ies,mods=~moda,method="DL")
preds.moda=predict(subganal.moda,newmods=c(0:2),transf=transf.ipft.hm,targ=list(ni=dat$total))
wi=1/sqrt(ies$vi)
size=1+3*(wi-min(wi))/(max(wi)-min(wi))
plot(ies$moda,transf.ipft.hm(ies$yi,targ=list(ni=dat$total)),cex=size,pch=1,las=1,xlab="Moderator A", ylab="Proportion")
...
ids=c(1:6)
pos=c(3)
text(ies$moda[ids],transf.ipft.hm(ies$yi,targ=list(ni=dat$total))[ids],ids,cex=0.9,pos=pos)
```
```{r Scatterplot for catergorical moderators}
...
wi=1/sqrt(ies$vi)
size=1+3*(wi-min(wi))/(max(wi)-min(wi))
plot(ies$moda,ies$yi,cex=size,pch=1,las=1,xlab="Moderator A", ylab="Double arcsine transformed proportion")
lines(0:2,preds.moda$pred,col="navy")
lines(0:2,preds.moda$ci.lb,lty="dashed",col="maroon")
lines(0:2,preds.moda$ci.ub,lty="dashed",col="maroon")
ids=c(1:6)
