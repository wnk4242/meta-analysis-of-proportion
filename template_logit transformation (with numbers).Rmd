...
pes.mal1=predict(pes.logit.mal1,transf=transf.ilogit,digits=5)
pes.logit.mal2=rma(yi,vi,data=ies.logit,subset=moderatora=="level2",method="DL")
pes.mal2=predict(pes.logit.mal2,transf=transf.ilogit,digits=5)
print(pes,digits=6)
print(pes.mal1,digits=6)
print(pes.mal2,digits=6)
```
...
addpoly(pes.mal1$pred,ci.lb=pes.mal1$ci.lb,ci.ub=pes.mal1$ci.ub,row=12.7,digits=6,cex=0.7,mlab="")
addpoly(pes.mal2$pred,ci.lb=pes.mal2$ci.lb,ci.ub=pes.mal2$ci.ub,row=-2.5,digits=6,cex=0.7,mlab="")
...
text(-0.005,12.7,pos=4,cex=0.7,
     bquote(paste("Subtotal(Q = ",.(formatC(pes.logit.mal1$QE, digits=1, format="f")), ", df = ", .(pes.logit.mal1$k - pes.logit.mal1$p),", p = ", .(formatC(pes.logit.mal1$QEp, digits=2, format="f")),
     "; ", I^2, " = ",.(formatC(pes.logit.mal1$I2, digits=1, format="f")), "%",")")))
text(-0.005,-2.8,pos=4,cex=0.7,
     bquote(paste("Subtotal(Q = ",.(formatC(pes.logit.mal2$QE, digits=1, format="f")),", df = ", .(pes.logit.mal2$k - pes.logit.mal2$p), 
     ", p = ", .(formatC(pes.logit.mal2$QEp, digits=2, format="f")), 
     "; ", I^2, " = ",.(formatC(pes.logit.mal2$I2, digits=1, format="f")), "%",")")))
abline(h=-4)
```
